...
async def send_a_list_of_messages(sender,msgCount):
    ...
    messages = [ServiceBusMessage(f"Message in list {i+msgCount}") for i in range(50)]
...
async def run(msgCount):
    ...
    for i in range (1,100):
        async with ServiceBusClient.from_connection_string(
            ...
            sender = servicebus_client.get_queue_sender(queue_name="testqueue")
            time.sleep(1)
            print("*")
            #sender = servicebus_client.get_topic_sender(topic_name=TOPIC_NAME)
            async with sender:
                # Send one message
                await send_single_message(sender,i)
